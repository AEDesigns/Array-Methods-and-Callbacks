...
fifaData.filter(function homeTeam(arr){
    if(arr.Year === 2014){
        console.log('Home:', arr['Home Team Name'], arr["Home Team Goals"])
    }
});

fifaData.filter(function awayTeam(arr){
    if(arr.Year === 2014){
        console.log('Away:', arr['Away Team Name'], arr["Away Team Goals"])
    }
});

fifaData.filter(function finals(arr){
    if(arr.Stage === 'Final' && arr.Year === 2014){
...
function getWinners(cb, data) {
    let winners = cb(data).map((arr) =>  arr["Home Team Goals"] > arr["Away Team Goals"] ? arr["Home Team Name"] :  arr["Away Team Name"]);
    return winners
  };
